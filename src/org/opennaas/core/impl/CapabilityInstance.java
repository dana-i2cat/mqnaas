 ...
 * </p>
 * <p>
 * Provides the {@link IService}s for each {@link ICapability} interface
 * implemented by the represented capability class.
 * </p>
 */
public class CapabilityInstance extends AbstractInstance<ICapability>{
	...
	/**
	 * Determines and returns all capability interfaces implemented by the
	...
	public Collection<Class<? extends ICapability>> getCapabilities() {
		...
		if ( capabilityClasses == null ) {
			...
			for ( Class<?> interfaze : ClassUtils.getAllInterfaces(clazz) ) {
				...
				if ( interfaze.equals(ICapability.class) ) continue;
				
				// Ignore all interfaces that do not extend ICapability
				if ( !ICapability.class.isAssignableFrom(interfaze) ) continue;
	...
	/*
	 * Relays the call to a capability implementations (an OSGi service) to an
	 * OpenNaaS Service.
	 */
