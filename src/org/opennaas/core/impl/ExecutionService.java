...
public class ExecutionService implements IExecutionService {
	...
	public Object execute(IService service, Object[] parameters) {
		...
		for ( IObservationFilter filter : observationFilters.keySet() ) {
			if ( filter.observes(service, parameters) ) {
				...
				System.out.println(getClass().getSimpleName() + ": Observing filter " + filter + " MATCHES, executing '" + notifiedService + "' " + (asynchronously ? "asynchronously" : "synchronously"));
	...
	public void registerObservation(IObservationFilter filter, IService service) {
		if ( filter == null ) throw new IllegalArgumentException("Filter must be given.");
		if ( service == null ) throw new IllegalArgumentException("Service must be given.");
